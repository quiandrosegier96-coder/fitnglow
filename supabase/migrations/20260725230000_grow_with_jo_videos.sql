...
create index if not exists grow_with_jo_published_idx
on public.grow_with_jo_videos(is_published, created_at desc);

alter table public.grow_with_jo_videos enable row level security;

create policy "grow with jo authenticated read"
on public.grow_with_jo_videos
for select to authenticated
using (is_published = true or lower(coalesce(auth.jwt()->>'email', '')) = 'fitandglow.joyce@gmail.com');

create policy "grow with jo joyce insert"
on public.grow_with_jo_videos
for insert to authenticated
with check (lower(coalesce(auth.jwt()->>'email', '')) = 'fitandglow.joyce@gmail.com');

create policy "grow with jo joyce update"
on public.grow_with_jo_videos
for update to authenticated
using (lower(coalesce(auth.jwt()->>'email', '')) = 'fitandglow.joyce@gmail.com')
with check (lower(coalesce(auth.jwt()->>'email', '')) = 'fitandglow.joyce@gmail.com');

create policy "grow with jo joyce delete"
on public.grow_with_jo_videos
